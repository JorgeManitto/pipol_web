<!-- Navbar móvil -->
<nav class=" md:hidden bg-[#1a0a3e] sticky top-0 text-white p-4 flex justify-between items-start  z-30">
  {{-- <h1 class="text-2xl font-bold text-[#d4af6a]">Pipol</h1> --}}
  <a href="{{ route('dashboard') }}">
    <img src="{{ asset('/images/logo-v3-recorte.png') }}" alt="logo" style=" object-fit: contain;height: 52px;">
  </a>
...
<div id="mobile-sidebar"
  ...
  <nav class="flex flex-col space-y-2 p-4">
    ...
    <a href="{{ route('profile.show', ['id'=>auth()->check() ? auth()->id() : 0]) }}" class="flex items-center gap-3 p-2 rounded {{ request()->routeIs('profile.show') ? 'bg-[#ffffff55]' : 'hover:bg-[#ffffff55]' }}  sidebar-icon">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
